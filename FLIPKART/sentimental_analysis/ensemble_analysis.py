...
Outputs:
  ensemble_results.csv       - per-review ensemble labels and predicted ratings
  ensemble_summary.csv       - all method metrics side by side
  ensemble_metric_bar.png    - bar chart comparison
  ensemble_confusion_*.png   - confusion matrices for ensemble methods
  ensemble_reliability.png   - bootstrap prediction stability plot
"""
...
RESULTS_CSV = os.path.join(BASE_DIR, "comparison_results.csv")
OUT_DIR     = BASE_DIR
...
def load_results():
    if not os.path.exists(RESULTS_CSV):
        raise FileNotFoundError(f"Run compare_methods.py first: {RESULTS_CSV}")
    ...
    print(f"  Loaded {len(df)} reviews from comparison_results.csv")
    return df
...
def plot_confusion(df, label_col, method_name):
    ...
    plt.tight_layout()
    fname = os.path.join(OUT_DIR, f"ensemble_confusion_{method_name.replace(' ', '_')}.png")
...
def plot_metric_comparison(summary_df):
    ...
    plt.tight_layout()
    fname = os.path.join(OUT_DIR, "ensemble_metric_bar.png")
    plt.savefig(fname, dpi=150)
    plt.close()
    print(f"  Saved: {os.path.basename(fname)}")
...
def plot_rmse_comparison(summary_df):
    ...
    plt.tight_layout()
    fname = os.path.join(OUT_DIR, "ensemble_rmse_bar.png")
    plt.savefig(fname, dpi=150)
    plt.close()
    print(f"  Saved: {os.path.basename(fname)}")
...
def plot_bootstrap_stability(df, all_preds):
    ...
    ax = axes[0]
    ax.hist(df["bagging_pred_std"], bins=40, color="#8172B2", alpha=0.8, edgecolor="white")
    ...
    plt.tight_layout()
    fname = os.path.join(OUT_DIR, "ensemble_reliability.png")
    plt.savefig(fname, dpi=150)
    plt.close()
    print(f"  Saved: {os.path.basename(fname)}")
...
def plot_cumulative_ratings(cum_df):
    ...
    ax.legend()
    plt.tight_layout()
    fname = os.path.join(OUT_DIR, "cumulative_ratings.png")
    plt.savefig(fname, dpi=150)
    plt.close()
    print(f"  Saved: {os.path.basename(fname)}")
...
def main():
    ...
    cum_csv = os.path.join(OUT_DIR, "cumulative_ratings.csv")
    cum_df.to_csv(cum_csv, index=False)
    print(f"\n  Cumulative ratings saved -> {cum_csv}\n")
    ...
    # Save full results
    ens_csv = os.path.join(OUT_DIR, "ensemble_results.csv")
    df.to_csv(ens_csv, index=False)
    print(f"  Full results saved -> {ens_csv}")

    sum_csv = os.path.join(OUT_DIR, "ensemble_summary.csv")
